...
//!    work runs on core-owned threads with start/cancel/progress.
...
impl TaggingSession {
    ...
    pub fn start(&self, progress: Arc<dyn TaggingProgressListener>) -> Result<(), TaggingError> {
        ...
        let spawned = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                // Release the lock *before* the callback so a listener that
                // reacts by inspecting or restarting the session sees a
                // settled one.
...
mod tests {
    ...
    fn a_bad_pack_directory_is_rejected_by_the_inspector() {
        ...
        assert!(matches!(err, TaggingError::PackFileMissing { .. }), "{err:?}");
    }
}
