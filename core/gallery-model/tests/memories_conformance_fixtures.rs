...
fn memory_engine_fixture_still_records_the_known_oddities() {
    ...
    // 1. The ID's date is the LOCAL calendar day the memory is about. In Tokyo
    //    a memory about June 12 is called `onThisDay-2024-06-12`. It used to
    //    render the instant in GMT and say `onThisDay-2024-06-11` — landmine 2,
    //    fixed and regenerated by `_plans/10-widget-timezone-fix.md`.
    let s = scenario(&dump, "non-utc-timezone-asia-tokyo");
    assert_eq!(s.inputs.time_zone, "Asia/Tokyo");
    assert_eq!(s.expected[0].id, "onThisDay-2024-06-12");
    assert!(
        s.expected[0]
            .subtitle
            .as_deref()
...
fn scheduled_memories_fixture_pins_the_horizon_and_the_parity_invariant() {
    ...
    // The acceptance criterion, in EVERY zone since
    // `_plans/10-widget-timezone-fix.md`: the id pre-published for day N is the
    // id the live pipeline produces on day N, with identical content. It used
    // to hold in UTC alone — `computeScheduledMemories` walked days from local
    // midnight while the id was rendered in GMT, so every calendar-tied widget
    // item in Asia/Tokyo carried the previous day's date and the deep link did
    // not resolve.
    for s in &dump.scenarios {
        for day in &s.parity {
    ...
    // An id belongs to exactly one horizon day. Day +4 used to pre-publish the
    // very id day +3 generated live, with different photos behind it.
    for s in &dump.scenarios {
        let mut seen: BTreeMap<&str, i32> = BTreeMap::new();
        for item in &s.scheduled {
            if let Some(other) = seen.insert(item.memory.id.as_str(), item.day_offset) {
                panic!(
                    "{}: `{}` is pre-published for both +{other} and +{}",
                    s.name, item.memory.id, item.day_offset
                );
            }
        }
    }

    // Birthday ids are not date-qualified and were immune to the drift, which
    // is what made it survivable at all.
    let tokyo = dump
        .scenarios
        .iter()
    ...
    assert!(
        ...
        "birthday ids carry no date and must still match"
    );
}
