...
//! in `core/fixtures/memories-conformance/README.md`. The one exception is the
//! GMT-rendered memory id (landmines 2 and 3), deliberately fixed and
//! regenerated by `_plans/10-widget-timezone-fix.md`.
...
/// Landmine 2, fixed in `_plans/10`: the id's date is the **local** calendar
/// day the memory is about. In Tokyo the engine selects the June-12-local
/// photos and calls the memory `onThisDay-2024-06-12`; it used to render the
/// instant in GMT and answer `onThisDay-2024-06-11`.
#[test]
fn a_memory_id_names_the_local_day_it_is_about() {
    let dump = dump();
    let out = run(&dump, "non-utc-timezone-asia-tokyo");
    assert_eq!(out[0].id, "onThisDay-2024-06-12");
